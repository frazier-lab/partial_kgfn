...
r"""
Method for optimizing discrete knowledge gradient-based acquisition functions for function network
"""
...
def optimize_discrete_acqf_for_function_network(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        parent_bounds: Tensor of lower and upper bounds for the outputs of each node.
        return_best_only: If False, outputs the solutions corresponding to all
            random restart initializations of the optimization.
        impose_assump: If true, we impose the upstream-downstream restriction on the problem being considered.
    Returns:
        A tensor of suggested candidate and its correponding acquisition function value.
    ...
    if impose_assump:
        ...
        parent_nodes = acq_function.model.dag.get_parent_nodes(node_idx[0])
        active_indices = acq_function.model.active_input_indices[node_idx[0]]
        if parent_bounds[node_idx] is not None:
            all_bounds = torch.cat(
                (bounds[:, active_indices], parent_bounds[node_idx].to(torch.double)),
                dim=-1,
            ).to(torch.double)
        else:
...
def retrieve_available_evaluations(
    ...
    """
    parent_nodes = model.dag.get_parent_nodes(node_idx)
    Ys = []
